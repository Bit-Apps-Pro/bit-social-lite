...
export default function LinkedinShareNowTemplate() {
  ...
  const handleTextArea = (e: ChangeEvent<HTMLTextAreaElement>) => {
    setShareNowData((prevSchedule: ShareNowType) =>
      produce(prevSchedule, draftSchedule => {
        draftSchedule.templates.linkedin.content = e.target.value
      })
    )
  }
  ...
  const handleCheckBox = (name: 'isAllImages' | 'isLinkCard', value: boolean) => {
    setShareNowData((prevSchedule: ShareNowType) =>
      produce(prevSchedule, draftSchedule => {
        draftSchedule.templates.linkedin.isLinkCard = false
        draftSchedule.templates.linkedin.isAllImages = false
  ...
  const handleLink = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setShareNowData((prev: ShareNowType) =>
      produce(prev, draft => {
        draft.templates.linkedin.link = value
      })
    )
  }
  ...
  const handleChange: UploadProps['onChange'] = ({ fileList }) => {
    const currentState = produce(shareNowData, draft => {
      draft.templates.linkedin.media = fileList
    })
  ...
  const setWpMedia = () => {
    ...
    if (typeof wp !== 'undefined' && wp.media) {
      ...
      const previousSelection = shareNowData.templates.linkedin.media.map(mediaFile => mediaFile.uid)
      ...
      <Form layout="vertical">
        <Form.Item htmlFor="textBox" label={__('Post')}>
          <Input.TextArea
            ...
            value={shareNowData.templates.linkedin.content}
          />
        </Form.Item>
        <Space className="py-2">
          <ShareNowPostTypeCheckBox
            handleCheckBox={handleCheckBox}
            templateData={shareNowData.templates.linkedin}
            type={postCheckBoxType}
          />
        </Space>

        {shareNowData.templates.linkedin?.isLinkCard && (
          <Form.Item htmlFor="link" label={__('Link')}>
            <Input
              id="link"
              onChange={e => handleLink(e)}
              placeholder={__('https://example.com')}
              value={shareNowData.templates.linkedin.link}
            />
          </Form.Item>
        )}

        {!shareNowData.templates.linkedin?.isLinkCard &&
          shareNowData.templates.linkedin?.isAllImages && (
            <Upload
              fileList={shareNowData.templates.linkedin.media}
