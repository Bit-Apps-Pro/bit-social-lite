...
export default function FacebookShareNowTemplate() {
  ...
  const handleTextArea = (e: ChangeEvent<HTMLTextAreaElement>) => {
    setShareNowData((prevSchedule: ShareNowType) =>
      produce(prevSchedule, draftSchedule => {
        draftSchedule.templates.facebook.content = e.target.value
      })
    )
  }
  ...
  const handleCheckBox = (name: 'isAllImages' | 'isLinkCard', value: boolean) => {
    setShareNowData((prevSchedule: ShareNowType) =>
      produce(prevSchedule, draftSchedule => {
        draftSchedule.templates.facebook.isLinkCard = false
        draftSchedule.templates.facebook.isAllImages = false
  ...
  const handleLink = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setShareNowData((prev: ShareNowType) =>
      produce(prev, draft => {
        draft.templates.facebook.link = value
      })
    )
  }
  ...
  const handleChange: UploadProps['onChange'] = ({ fileList }) => {
    const currentState = produce(shareNowData, draft => {
      draft.templates.facebook.media = fileList
    })
  ...
  const setWpMedia = () => {
    ...
    if (typeof wp !== 'undefined' && wp.media) {
      const imgSelectionFrame = wp.media({
        ...
        multiple: isProClient ? 'add' : false,
        title: __('Media')
      })

      const previousSelection = shareNowData.templates.facebook.media.map(mediaFile => mediaFile.uid)
      ...
      <Form layout="vertical">
        <Form.Item htmlFor="textBox" label={__('Post')}>
          <Input.TextArea
            ...
            value={shareNowData.templates.facebook.content}
          />
        </Form.Item>
        <Space className="py-2">
          <ShareNowPostTypeCheckBox
            handleCheckBox={handleCheckBox}
            templateData={shareNowData.templates.facebook}
            type={postCheckBoxType}
          />
        </Space>

        {shareNowData.templates.facebook?.isLinkCard && (
          <Form.Item htmlFor="link" label={__('Link')}>
            <Input
              id="link"
              onChange={e => handleLink(e)}
              placeholder={__('https://example.com')}
              value={shareNowData.templates.facebook.link}
            />
          </Form.Item>
        )}

        {!shareNowData.templates.facebook?.isLinkCard &&
          shareNowData.templates.facebook?.isAllImages && (
            <Upload
              fileList={shareNowData.templates.facebook.media}
