...
const getSecret = async (tumblrCredentialData: CredentialsType) => {
  const requestData = {
    client_id: tumblrCredentialData.appId,
    platform: 'tumblr'
  }
  const options = {
...
const getToken = async (
  authAppData: CredentialsType,
  authCodeResponse: AuthCodeResponseType,
  authType: string
) => {
  const appData = { ...authAppData }
...
export default tumblrAppAuthorization
