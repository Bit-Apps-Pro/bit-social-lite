...
const getToken = async (appData: CredentialsType, authCodeResponse: AuthCodeResponseType) => {
  ...
  if (status === 'success') {
    const pinterestConnectState: Partial<AccountConnectType> = {
      details: data,
      isModalOpen: true,
      loading: false,
      platform: 'pinterest',
    ...
    setAtom($accountConnect, pinterestConnectState)
  }
}
...
const pinterestAppAuthorization = (
  appData: CredentialsType,
  handleConnectCallback: () => void,
...
export default pinterestAppAuthorization
