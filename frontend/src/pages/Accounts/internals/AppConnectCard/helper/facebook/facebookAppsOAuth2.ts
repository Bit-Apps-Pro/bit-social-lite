...
const getSecret = async (facebookCredentialData: CredentialsType) => {
  const requestData = {
    client_id: facebookCredentialData.appId,
    platform: 'facebook'
  }
  const options = {
...
const getToken = async (
  ...
  if (status === 'success') {
    const facebookConnectState: Partial<AccountConnectType> = {
      details: data,
      isModalOpen: true,
      loading: false,
      platform: 'facebook',
    ...
    setAtom($accountConnect, facebookConnectState)
  }
}
...
const facebookAppAuthorization = (
  appData: CredentialsType,
  handleConnectCallback: () => void,
  ...
  const scope =
    'pages_show_list,pages_read_engagement,pages_manage_metadata,pages_manage_posts,business_management'
...
export default facebookAppAuthorization
