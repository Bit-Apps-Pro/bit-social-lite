...
export default function ProLineSettings() {
  ...
  const handleChange: KeyedValueHandler<SocialTemplates['line']> = (key, val) => {
    setTemplates(prev =>
      produce(prev, draft => {
        draft.line[key] = val
      })
  ...
  return (
    <Card style={{ backgroundColor: token.colorFillAlter }}>
      ...
      <Row gutter={20}>
        <Col span={14}>
          <Title level={4}>{__('Line Template Settings')} </Title>
          <TemplateDocLink platform="Line" />
            ...
            <Flex gap={20} justify="space-between">
              ...
              <div>
                <Select
                  ...
                  style={{ minWidth: 200 }}
                  value={templates.line.postingType}
                />
              </div>
            </Flex>
          </Card>
          ...
          <Card style={{ marginTop: 10 }}>
            <Flex gap={20} justify="space-between">
              <Card.Meta
                description={sprintf(
                  __(
                    'Line restricts the length of a post to %1$d characters. If you enable this option, the first %1$d characters of your personalized message will be shared; if not, the limit prevents the post from being shared'
                  ),
                  platformsLimitations.line.content.length
                )}
                title={__('Trim Message')}
              />
              <div>
                <Switch
                  checked={templates.line.trimMessage}
