...
export default function ProTwitterSettings() {
  ...
  const handleChange: KeyedValueHandler<SocialTemplates['twitter']> = (key, val) => {
    setTemplates(prev =>
      produce(prev, draft => {
        draft.twitter[key] = val
      })
    )
  }
        ...
        <Card size="small" style={{ backgroundColor: token.colorFillAlter }}>
          ...
          <Card style={{ marginTop: 10 }}>
            <Flex gap={20} justify="space-between">
              ...
              <div>
                <Switch
                  checked={templates.twitter.trimMessage}
