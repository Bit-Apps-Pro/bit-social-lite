...
export default function ProDiscordSettings() {
  ...
  const handleChange: KeyedValueHandler<SocialTemplates['discord']> = (key, val) => {
    setTemplates(prev =>
      produce(prev, draft => {
        draft.discord[key] = val
      })
    )
  }
    ...
    <Row gutter={20}>
      ...
      <Col span={14}>
        <Card size="small" style={{ backgroundColor: token.colorFillAlter }}>
          <Title level={4}>{__('Discord Template Settings')}</Title>
          <TemplateDocLink platform="Discord" />
          ...
          <Card>
            ...
            <div>
              <MessageBox
                ...
                rows={5}
                style={{ minWidth: 200 }}
                value={templates.discord.content}
              />
            </div>
          </Card>
            ...
            <Flex gap={20} justify="space-between">
              ...
              <div>
                <Select
                  ...
                  style={{ minWidth: 200 }}
                  value={templates.discord.postingType}
                />
              </div>
            </Flex>
          </Card>
          ...
          <Card style={{ marginTop: 10 }}>
            <Flex gap={20} justify="space-between">
              <Card.Meta
                description={`Discord restricts the length of a tweet to ${platformsLimitations.discord.description.length} characters. If you enable this option, the first ${platformsLimitations.discord.description.length} characters of your personalized message will be shared; if not, the limit prevents the tweet from being shared.`}
                title={__('Trim Message')}
              />
              <div>
                <Switch
                  checked={templates.discord.trimMessage}
