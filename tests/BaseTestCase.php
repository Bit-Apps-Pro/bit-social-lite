...
abstract class BaseTestCase extends WP_Ajax_UnitTestCase
{
    ...
    private function callAjax($method, $route, $data)
    {
        ...
        if (is_readable(dirname(__DIR__) . DIRECTORY_SEPARATOR . 'backend' . DIRECTORY_SEPARATOR . 'routes' . DIRECTORY_SEPARATOR . 'ajax.php')) {
            echo 'ajax.php exists';
            $router = new Router(RequestType::AJAX, Config::VAR_PREFIX, '');
            $router->setMiddlewares(Plugin::instance()->middlewares());
            include dirname(__DIR__) . DIRECTORY_SEPARATOR . 'backend' . DIRECTORY_SEPARATOR . 'routes' . DIRECTORY_SEPARATOR . 'ajax.php';
            $router->register();
        }
